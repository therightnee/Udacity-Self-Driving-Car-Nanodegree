    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          //Invoke Twiddle Algorithm here
          if (tune == true && step_count >= step_threshold) {
              int cur_gain = floor(loop_counter % 3);
              std::cout << "Tuning" << std::endl;
              tol_check = pid.TwiddleTuner(cte, steer_value, step_count, 
                    cur_gain);
